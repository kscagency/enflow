...
const MOCK_NOTIFICATIONS: Notification[] = [
  { id: '1', client_id: 'preview-client', agency_id: null, message: 'Your March campaign report is ready to view.', type: 'report_ready', is_read: false, created_at: new Date().toISOString() },
  { id: '2', client_id: 'preview-client', agency_id: null, message: '3 new files added to your Assets folder.', type: 'file_uploaded', is_read: false, created_at: new Date(Date.now() - 86400000).toISOString() },
  { id: '3', client_id: 'preview-client', agency_id: null, message: 'Your February campaign report was uploaded.', type: 'report_ready', is_read: true, created_at: new Date(Date.now() - 86400000 * 15).toISOString() },
]

const FolderIcon = () => (
  <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
    <path d="M1 3.5A1.5 1.5 0 012.5 2h2l1 1.5H9.5A1.5 1.5 0 0111 5v4A1.5 1.5 0 019.5 10.5h-7A1.5 1.5 0 011 9V3.5z" fill="#0A7B7B" opacity="0.7"/>
  </svg>
)
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
          ...
          <div style={{ background: '#fff', border: '0.5px solid #e0e3e0', borderRadius: '12px', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '14px 18px' }}>
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                {[
                  ...
                  <div key={folder.name} style={{
                    ...
                    background: '#F5F6F4', border: '0.5px solid #e0e3e0', borderRadius: '8px',
              ...
              <div style={{ marginTop: '12px' }}>
                <Link href="/portal/files" style={{
                  display: 'inline-block', fontSize: '12px', fontWeight: 500, color: '#fff',
                  background: '#0A7B7B', borderRadius: '8px', padding: '8px 14px',
                  ...
                  <span style={{ position: 'relative' }}>View files</span>
                </Link>
              </div>
            </div>
          </div>
            ...
            <div style={{ padding: '14px 18px' }}>
              <div style={{ background: '#F5F6F4', borderRadius: '8px', padding: '14px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
                <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
                  <div>
                    <p style={{ fontFamily: "'Bricolage Grotesque', sans-serif", fontSize: '20px', fontWeight: 700, color: '#080C0C', letterSpacing: '-0.01em', margin: '0 0 2px' }}>12,480</p>
                    <p style={{ fontSize: '11px', color: '#888', margin: 0 }}>Organic sessions</p>
                  </div>
                  <div>
                    <p style={{ fontFamily: "'Bricolage Grotesque', sans-serif", fontSize: '20px', fontWeight: 700, color: '#080C0C', letterSpacing: '-0.01em', margin: '0 0 2px' }}>284</p>
                    <p style={{ fontSize: '11px', color: '#888', margin: 0 }}>Leads generated</p>
                  </div>
                  <div>
                    <p style={{ fontFamily: "'Bricolage Grotesque', sans-serif", fontSize: '20px', fontWeight: 700, color: '#080C0C', letterSpacing: '-0.01em', margin: '0 0 2px' }}>$18.40</p>
                    <p style={{ fontSize: '11px', color: '#888', margin: 0 }}>Cost per lead</p>
                  </div>
                </div>
                <Link href="/portal/reports" style={{
                  fontFamily: "'Instrument Sans', sans-serif",
                  fontSize: '12px', fontWeight: 500, color: '#fff',
                  background: '#0A7B7B', border: 'none', borderRadius: '8px',
                  padding: '8px 14px', cursor: 'pointer', whiteSpace: 'nowrap',
                  textDecoration: 'none', position: 'relative', overflow: 'hidden', display: 'inline-block',
                }}>
                  <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '0.5px', background: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.4) 50%, transparent)' }} />
          ...
          <div style={{ gridColumn: '1 / -1', background: '#fff', border: '0.5px solid #e0e3e0', borderRadius: '12px', overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 18px 12px', borderBottom: '0.5px solid #f0f0ee' }}>
              ...
              <Link href="/portal/notifications" style={{ fontSize: '12px', fontWeight: 500, color: '#0A7B7B', cursor: 'pointer', textDecoration: 'none' }}>
                View all
              </Link>
            </div>
            <div>
              {notifications.slice(0, 3).map((n, i) => {
                const isLast = i === Math.min(notifications.length, 3) - 1
                return (
                  <div
                    key={n.id}
                    style={{
                      ...
                      borderLeft: !n.is_read ? '2.5px solid #0A7B7B' : 'none',
                      background: !n.is_read ? 'rgba(10,123,123,0.02)' : 'transparent',
                    }}
                  >
                    <div style={{
                      width: '7px', height: '7px', borderRadius: '50%',
                      background: !n.is_read ? '#0A7B7B' : 'transparent',
                      marginTop: '4px', flexShrink: 0,
                    }} />
                    <div>
                      <p style={{ fontSize: '13px', color: n.is_read ? '#888' : '#080C0C', margin: '0 0 3px', lineHeight: 1.4 }}>
                        {n.message}
                      </p>
